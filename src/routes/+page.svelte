<script lang="ts">
    ...
    let isImageProcessed = $state(true);

    async function createPop(event: Event) {
...
<main class="p-4 flex items-center flex-col gap-12 pt-12">
    ...
    <div>
        <img src={imageUrl} class="max-w-60 max-h-40"/>
        <div class="flex justify-center items-center">
            <div 
                ...
                on:dragover={() => console.log('hello, world')}
                on:drop={createPop}
            >
            ...
            {#if isImageProcessed}
                ...
                <div class="rounded-full bg-green-300 w-60 h-60"></div>
            {/if}
            {#if loading}
                <div role="status">
